...
const {byUrl, analyzing, analyzeAll, generate} = useStructuredData()
...
const missingCount = computed(() => urlList.value.filter(u => byUrl[u]?.status?.needsGeneration).length)
...
<template>
  <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6">
    <div class="flex items-center justify-between mb-1">
      ...
      <button
          :disabled="analyzing || !urlList.length"
          ...
          @click="runAnalysis"
      >
        {{ analyzing ? 'Analyse…' : (analyzed ? 'Réanalyser' : 'Analyser les données structurées') }}
      </button>
    </div>
    ...
    <p v-if="analyzed && !settings.isConfigured" class="text-xs text-gray-500 dark:text-gray-400 mb-3">
    ...
    <ul v-if="analyzed" class="divide-y divide-gray-100 dark:divide-gray-700">
