    ...
    <div :class="{ 'has-error': error, 'is-valid': isValidUrl && !error }" class="input-wrapper">
      ...
      >
        <template v-if="loading">
          ...
          <span class="ml-2">{{ $t('urlInput.analyzing') }}</span>
        </template>
        <template v-else>
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path d="M9 5l7 7-7 7" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/>
          </svg>
          <span class="ml-2">{{ $t('urlInput.analyze') }}</span>
        </template>
      </button>
    </div>

    <p v-if="error" class="error-message">{{ error }}</p>
    <p v-else-if="inputValue && !isValidUrl" class="hint-message">{{ $t('urlInput.invalidHint', { example: 'https://example.com' }) }}</p>
  </div>
</template>
