...
const summary = computed(() => {
  ...
  const withLatest = stats.filter(s => s.latest)

  const sovValues = withLatest
      .map(s => s.latest.shareOfVoice)
      .filter(v => typeof v === 'number')
  const avgSov = sovValues.length
      ? Math.round(sovValues.reduce((a, b) => a + b, 0) / sovValues.length)
      : null

  const absent = withLatest.filter(s => !s.latest.brandMentioned).length

  return {
    total: geoStore.count,
    avgSov,
    absent,
    neverRun: stats.filter(s => !s.latest).length
  }
})
...
async function handleAdd() {
  ...
  const item = geoStore.addItem({
    prompt: newPrompt.value,
    brand: newBrand.value,
    competitors: newCompetitors.value
  })
  if (!item) {
...
async function handleRun(item) {
  const result = await runPrompt(item)
...
<template>
  <div class="min-h-screen flex flex-col">
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <button
                ...
                title="Être alerté quand la visibilité de la marque change"
    ...
    <main class="flex-1 max-w-6xl w-full mx-auto px-4 py-8">
      ...
      >
        Aucun fournisseur LLM configuré. Renseignez une clé d'API dans les
        <router-link class="underline font-medium" to="/settings">Paramètres</router-link>
        pour interroger les moteurs IA.
      </div>
      ...
      <div v-if="!geoStore.isEmpty" class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Marque absente</p>
      ...
      <div v-if="geoStore.isEmpty" class="text-center py-20">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto">
          ...
          marque et vos concurrents, puis mesurez votre visibilité dans les réponses des IA.
        </p>
      </div>
